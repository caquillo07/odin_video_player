...
init_sdl2 :: proc() -> (^sdl2.Window, ^sdl2.Renderer, ^sdl2.Surface) {
    ...
    surf := sdl2.CreateRGBSurface(
        cast(u32)sdl2.WINDOW_SHOWN,
        400,
        400,
        32,
        0xff000000,
...
decode_packet :: proc(packet: ^types.Packet, codec_ctx: ^types.Codec_Context, frame: ^types.Frame, idx: int) -> i32 {
    ...
    if avutil.av_error(response) == types.AVError.EOF || avutil.av_error(response) == types.AVError.EAGAIN {
        return response
    } else if response < 0 {
...
render_frame :: proc(rend: ^sdl2.Renderer, surf: ^sdl2.Surface, frame: ^types.Frame) {
    ...
    tex := sdl2.CreateTexture(
        rend,
        cast(u32)sdl2.PixelFormatEnum.IYUV,
